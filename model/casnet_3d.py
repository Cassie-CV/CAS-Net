...
class AGFF(nn.Module):
    ...
    def forward(self, low_feat, high_feat):
        ...
        # Reweight low-level features
        reweighted = att[:, :low_feat.size(1), :, :, :] * low_feat
        return reweighted + high_feat
...
class SAFE(nn.Module):
    ...
    def forward(self, x):
        ...
        att = self.softmax(torch.bmm(Q.permute(0, 2, 1), K))
...
class MSFA(nn.Module):
    def __init__(self, in_channels_list):
        ...
        self.att_convs = nn.ModuleList([nn.Conv3d(8, 8, kernel_size=3, padding=1) for _ in in_channels_list])
        self.sigmoid = nn.Sigmoid()
        self.out_conv = nn.Conv3d(8, 1, kernel_size=3, padding=1)
    ...
    def forward(self, features):
        ...
        for i in range(len(compressed) - 2, -1, -1):
            ...
            att = self.sigmoid(self.att_convs[i](torch.cat([compressed[i], up], dim=1)))
...
class CASNet3D(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decoder + AGFF
        d4 = self.decoder4(torch.cat([self.agff4(e3, F.interpolate(e4, scale_factor=2)), self.deconv4(e4)], dim=1))
        d3 = self.decoder3(torch.cat([self.agff3(e2, F.interpolate(d4, scale_factor=2)), self.deconv3(d4)], dim=1))
        d2 = self.decoder2(torch.cat([self.agff2(e1, F.interpolate(d3, scale_factor=2)), self.deconv2(d3)], dim=1))
        d1 = self.decoder1(torch.cat([self.agff1(e0, F.interpolate(d2, scale_factor=2)), self.deconv1(d2)], dim=1))

        # MSFA
        out = self.msfa([d4, d3, d2, d1])
        out = self.final(out)
